 ...
 * @tagList<必填><array>                
 * @business_tag_relation<必填><array>  对应关系在由服务端维护
...
const {{name}} = {
  ...
  customData: {},
  business_tag_relation: {{business_tag_relation}}
}
</custom>
`
        ...
        case 'src/index.vue':
          mkdir(`./packages/${name}/src`, function () {
            createfile(pathName, render(INDEX_TEMP, {
              ...
              label: `"${answers.label}"`,
              business_tag_relation: json5.stringify([
                {
                  business : answers.businessList,
                  tagList : answers.tagList
                }
              ])
            }))
          })
