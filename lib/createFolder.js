 ...
 * @technologyVersion<必填><string>   当 technologyStack !=Js 时, 此项填写技术转版本，例： 2.6.0
 * @businessList<必填><array>         可多选， 选项为：{{BUSINESS}}
 * @applyList<必填><string>           神策event_id：
 * @thumbnail<选填><string>           组件预览图
...
async function createComponent (nameAndVersion) {
  const name = nameAndVersion.split('/')[0]
  const version = nameAndVersion.split('/')[1] || '0.0.1'
      ...
      switch (file) {
        ...
        case 'example/exam1.vue':
          mkdir(`./packages/${name}/example`, function () {
            createfile(pathName, render(EXAMPLE_TEMP,{
              demo: createTag(name)
            }))
          })
          break;
...
function createTag (name) {
  let _name = name.split('').map((item, index)=> {
    if(index === 0) {
  ...
  _name = _name.replace(/[A-Z]/g, (item) => {
    return '-' + item.toLocaleLowerCase()
  })
